...
public class PlayerLuta : MonoBehaviour
{
    ...
    public static PlayerLuta current;
    ...
    void Update()
    {
     ...
     if((!isDead) && (!EnemyJoaoVindo.current.isDead) && (Time.timeScale == 1) )
     {   
        
        
        if((Input.GetKey(KeyCode.D)) && (!isDefense) && (isAtk == false) && (Controller.current.canPlay == true) && (!isPower) && (!EnemyJoaoVindo.current.isDead) && (EnemyJoaoVindo.current.GetComponent<EnemyJoaoVindo>().enabled) ) 
        {
        ...
        if( (Input.GetKey(KeyCode.A)) && (!isDefense) && (isAtk == false) && (Controller.current.canPlay == true) && (!isPower) && (!EnemyJoaoVindo.current.isDead) && (EnemyJoaoVindo.current.GetComponent<EnemyJoaoVindo>().enabled) )
        {
            rig.velocity = new Vector2(-Speed * Time.deltaTime, rig.velocity.y);
            anim.SetBool("isWalking", true);
        ...
        {
            if(Input.GetKeyDown(KeyCode.M) && (EnemyJoaoVindo.current.GetComponent<Rigidbody2D>().bodyType == RigidbodyType2D.Dynamic) )
            {
                //AudioPunch.Play();
