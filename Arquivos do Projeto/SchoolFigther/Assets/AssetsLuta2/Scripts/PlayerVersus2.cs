...
public class PlayerVersus2 : MonoBehaviour
{
    ...
    public static PlayerVersus2 current;
    ...
    void Update()
    {
     ...
     if((!isDead) && (!PlayerVersus1.current.isDead) && (Time.timeScale == 1) )
     {   
        
        
        if((Input.GetKey(KeyCode.RightArrow)) && (!isDefense) && (isAtk == false) && (ControlVersus.current.canPlay == true) && (!isPower) && (!PlayerVersus1.current.isDead) && (PlayerVersus1.current.GetComponent<PlayerVersus1>().enabled) ) 
        {
        ...
        if( (Input.GetKey(KeyCode.LeftArrow)) && (!isDefense) && (isAtk == false) && (ControlVersus.current.canPlay == true) && (!isPower) && (!PlayerVersus1.current.isDead) && (PlayerVersus1.current.GetComponent<PlayerVersus1>().enabled) )
        {
            rig.velocity = new Vector2(-Speed * Time.deltaTime, rig.velocity.y);
            anim.SetBool("isWalking", true);
        ...
        {
            if(Input.GetKeyDown(KeyCode.Keypad4) && (PlayerVersus1.current.GetComponent<Rigidbody2D>().bodyType == RigidbodyType2D.Dynamic) )
            {
                //AudioPunch.Play();
        ...
        /*DEFESA*/
        if((Input.GetKeyDown(KeyCode.DownArrow)) && (!isDefense))
        {
        ...
        /*SOLTAR ENERGIA*/
        if ((Input.GetKeyDown(KeyCode.Keypad5)) && (!isJumping) && ((BarraEnergy.current.Energy >= BarraEnergy.current.gastoEnergy) || (BarraEnergy.current.Energ >= 20f)) )
        {    
        ...
        /*SOLTAR ULTIMATE*/
        if ((Input.GetKeyDown(KeyCode.Keypad6)) && (!isJumping) && (BarraEnergy.current.Energ >= 60f) )
        {
    ...
    public void TakeDamage(float FullDamage)
    {
        ...
        if(LifePlayer2 <= 0)
        {
            ...
            //DestroyBody();
            PlayerVersus1.current.DestroyBody();
